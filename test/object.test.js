...
{
  const actual = inspect({ foo: true }, { objectConstructor: true, useNew: true })
...
{
  const actual = inspect({ name: "dam" }, { singleQuote: true })
  const expected = `{
  'name': 'dam'
}`
  assert({
...
{
  const actual = inspect({ foo: true, nested: { bar: true } }, { parenthesis: true })
  const expected = `({
  "foo": true,
  "nested": ({
    "bar": true
